...
async def sheets_cell_values(
    ...
    persistent_ctx = getattr(evaluate, 'env', None)
    if persistent_ctx is None:
        logger.error("No persistent context available in evaluate hub")
        return EvaluationResult(
            reward=0.0,
            done=False,
            content="No persistent context available",
            info={"error": "No persistent context available"},
        )
